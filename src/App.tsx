...
const drawExample = async () => {
  ...
  const { sciChartSurface, sciChartOverview, controls } =
    await createCandlestickChart(divElementId, divOverviewId);
  ...
  const priceBars = await simpleBinanceRestClient.getCandles(
    "BTCUSDT",
    "1m",
    startDate,
    endDate
  ...
  controls.setData("M5", "5 Minute", priceBars);
  ...
  const subscription = binanceSocketClient
    .getRealtimeCandleStream("BTCUSDT", "1m")
    .subscribe((pb) => {
      const priceBar = {
        date: pb.openTime,
        open: pb.open,
...
export default function RealtimeTickingStockCharts() {
  ...
  React.useEffect(() => {
    (async () => {
      const { sciChartSurface, sciChartOverview, subscription, controls } =
        await drawExample();
