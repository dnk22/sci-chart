...
} from "scichart-example-dependencies";
import classes from "./styles/Examples.module.scss";
import { getChartDataFromAPI } from "./api";
...
const drawExample = async () => {
  ...
  SciChartSurface.setRuntimeLicenseKey(
    ""
  );
  ...
  sciChartSurface.yAxes.add(
    new NumericAxis(wasmContext, {
      growBy: new NumberRange(0.000001, 0.000001),
  ...
  const priceBars = await getChartDataFromAPI();
  ...
  priceBars.forEach((priceBar: any) => {
    const keys = Object.keys(priceBar);
    const firstKey: any = keys[0];
    xValues.push(+firstKey / 1000);
    openValues.push(priceBar[firstKey][0]);
    highValues.push(priceBar[firstKey][1]);
    lowValues.push(priceBar[firstKey][2]);
    closeValues.push(priceBar[firstKey][3]);
    volumeValues.push(
      +priceBar[firstKey][0] +
        priceBar[firstKey][1] +
        priceBar[firstKey][2] +
  ...
  xAxis.visibleRange = new NumberRange(
    startViewportRange.getTime() / 1000,
...
const getOverviewSeries = (defaultSeries: IRenderableSeries): any => {
...
export default function CandlestickChart() {
  ...
  const [candlestickChartSeries, setCandlestickChartSeries] = React.useState<
    FastCandlestickRenderableSeries | any
  >();
  const [ohlcChartSeries, setOhlcChartSeries] = React.useState<
    FastOhlcRenderableSeries | any
  >();

  async function renderChart() {
    const { sciChartSurface, overview, candlestickSeries, ohlcSeries } =
      await drawExample();
  ...
  // Function to fetch data and push to array, called every 5 minutes
  async function fetchDataAndUpdateArray() {
    return renderChart()
      .then(() => {
        // Schedule the next API call after x minutes
        setTimeout(fetchDataAndUpdateArray, time * 60 * 1000);
        return true;
      })
      .catch((error) => {
        console.error("Error occurred during API call:", error);
        // If an error occurs, retry after x minutes
        setTimeout(fetchDataAndUpdateArray, time * 60 * 1000);
        return false;
      });
  }
  ...
  return (
    <React.Fragment>
      <div
        className={classes.FullHeightChartWrapper}
        style={{ background: appTheme.DarkIndigo }}
      >
        <ToggleButtonGroup
